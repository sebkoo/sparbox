...
| M1a — the demo path              | 15        | in progress |
...
- The interview domain as validated schemas, and the import boundary that keeps the interview loop
  from reaching the screens, the stores, or the validator
...
- The session and settings stores, landing after the runner rather than with the schemas, so the
  state library arrives in the commit that first drives it instead of six commits before
